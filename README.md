...
Lors du premier "git push -u origin main", l'erreur suivante est apparue :

       error: src refspec main does not match any

Cause : la branche locale s'appelait encore "master" (nom par defaut de
Git), alors que le push ciblait une branche nommee "main" qui n'existait
pas localement.

Solution : renommer la branche locale avant de pousser :
...
---------------------------
La difficulte de cette situation vient du fait que le projet existe deja
localement avec un historique. Deux approches etaient envisageables :

   - Cloner un depot GitHub vide puis y recopier les fichiers a la main.
     Cette approche fonctionne, mais elle FAIT PERDRE tout l'historique
     des commits deja realises : le projet repartirait d'un seul commit
     initial.

   - Lier le depot local existant au depot distant via "git remote add",
     puis pousser l'historique complet. C'est l'approche retenue, car
     elle preserve l'integralite de l'historique des versions, ce qui est
     precisement l'interet d'un gestionnaire de versions.

L'ajout du membre B comme collaborateur (plutot que de simplement rendre
le depot public) est necessaire parce qu'un depot public autorise la
lecture, mais pas l'ecriture : sans le statut de collaborateur, le
membre B obtiendrait une erreur 403 lors de son premier push.
